...
function ButtonResume() {
  return (
    <a
      ...
      className={mc("button button--ghost px-2", "md:button--big md:px-2")}
    >
      <DocumentIcon className={mc("h-5 w-5")} />
      RESUME
    </a>
  );
}
...
function AvailableForHire() {
  return (
    ...
    >
      <span className={mc("relative flex h-2 w-2")}>
        <span
          className={mc(
...
function HeaderCta({
  ...
  return (
    <m.div className={mc("flex gap-2")} initial="hide" animate="show">
      <m.div
        className={mc("relative z-20")}
        variants={animation}
      ...
      {isFree ? (
        ...
        >
          ...
          <m.div
            className={mc("absolute top-0 left-0")}
